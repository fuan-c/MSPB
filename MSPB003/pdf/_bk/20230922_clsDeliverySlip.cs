...
namespace MSPB.MSPB003.pdf
{
    ...
    class clsDeliverySlip
    {
        ...
        daofrmMSPB003 dao = new daofrmMSPB003("DBConnection");
        ...
        public string CreateDeliverySlip()
        {
            ...
            ovlFile = ConfigurationManager.AppSettings["MSPB003_YU_PACK_DELIVERY_SLIP_OVL"];
            
            outPath = Path.Combine(ConfigurationManager.AppSettings["MSPB003_YU_PACK_DELIVERY_SLIP_OUTPUT_PATH"], this.SHIPPING_DATE);
            ...
            _logger.DebugFormat("DataOvl:{0}", ovlFile);

            Dictionary<string, List<clsDeliverySlipRecord>> dicSlipData = this.lstDeliverySlipData
                                        .GroupBy(x => x.SHIPPING_DATE)
                                    .ToDictionary(keys => keys.Key, grp => grp.ToList());

            using (PdfCreator pdf = new PdfCreator(outFile, ovlFile))
            {
                int keyCount = 0;
                ...
                foreach (string key in dicSlipData.Keys)
                {
                    ...
                    foreach (clsDeliverySlipRecord info in lstSlipData)
                    {
                        ...
                        try
                        {
                            ...
                            DataTable dt = dao.Select_M_SORTING_CODE_PB(info.SEND_DEST_POSTAL_CODE);
                            if (dt.Rows.Count <= 0)
                            {
                                info.SORTING_NO = "";
                            }
                            ...
                            {
                                info.SORTING_NO = dt.Rows[0]["SORTING_NO"].ToString();
                            }
                            
                        }
                        //logger.Debug("検索が完了しました。");
                        catch (Exception ex)
                        {                            
                            _logger.Fatal("仕訳コードマスタテーブルから仕分け番号取でエラーが発生しました。");
