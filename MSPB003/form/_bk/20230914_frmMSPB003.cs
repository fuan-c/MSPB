...
namespace MSPB.MSPB003.form
{
    ...
    class frmMSPB003 : Form
    {
        ...
        private void btnSlipPrintClick(object sender, EventArgs e)
        {
            ...
            {
                _logger.Info("テーブルから対象データ取得でエラーが発生しました。");
                _logger.Info("エラー内容：" + ex.Message);
                _logger.Info("出荷日指定の日付存在チェック：終了");
        ...
        private void setSendSlipTargeCntDisplay()
        {
            ...
            {   
                ...
                lblNonApprovedCnt.Text = dtNonApproved == null ? "0" : dtNonApproved.Rows.Count.ToString("#,0");
        ...
        private void registMSShippingQMS()
        {
            ...
            try
            {
                ...
                foreach (DataRow row in dtContract.Rows)
                {
                    ...
                    // 発送QMSのMAX ID取得
                    int maxId = dao.Select_MaxID_T_MS_PB_SHIPPING_QMS();
                    ...
                    // 発送QMSテーブルデータ登録
                    dao.Insert_ContractorRtn_T_MS_PB_SHIPPING_QMS(qmsData);
            ...
            catch (Exception ex)
            {
                ...
                else
                {
                    // ロールバック
                    dao.Rollback();
                    _logger.Info("MS発送_QMSテーブル登録処理でエラーが発生しました。");
                    _logger.Info("エラー内容：" + ex.Message);
                    new clsMessageBox().MessageBoxOKOnly("MS発送_QMSテーブル登録処理でエラーが発生しました。" + Environment.NewLine + ex.Message.ToString() + Environment.NewLine + "開発部門に問い合わせ願います。", "エラー", MessageBoxIcon.Error);
                    throw;
                }                
            }
            finally
        ...
        private void outputShippingSlip()
        {
            ...
            try
            {
                ...
                string shppingCheckListFile = string.Empty;
                ...
                // 契約者返却発送リスト出力
                shppingCheckListFile = new clsMSRtnShippingList()
                {
                ...
                // 発送帳票出力済みデータのエスカレテーブル更新処理
                updateEscalationTBL();

                // 処理履歴更新
                setProcessHistoryTbl("ゆうパック配送伝票印字", Path.GetFileName(yuPackChekFile), dtContract.Rows.Count);
                setProcessHistoryTbl("伝票チェックシート", Path.GetFileName(checkSheetFile), dtContract.Rows.Count);

                // 終了メッセージ                
                new clsMessageBox().MessageBoxOKOnly("発送帳票出力が完了しました", "完了", MessageBoxIcon.Information);
                this.Cursor = Cursors.WaitCursor;
                // 印刷画面を表示する
                new comPdfFile().pdfPrintOpen(shppingListFile);
                new comPdfFile().pdfPrintOpen(checkSheetFile);
                new comPdfFile().pdfPrintOpen(shppingCheckListFile);
            ...
            catch (Exception ex)
            {
                ...
                {
                    _logger.Info("帳票出力処理でエラーが発生しました。");
                    _logger.Info("エラー内容：" + ex.Message);
                    new clsMessageBox().MessageBoxOKOnly("帳票出力処理でエラーが発生しました。" + Environment.NewLine + ex.Message.ToString() + Environment.NewLine + "開発部門に問い合わせ願います。", "エラー", MessageBoxIcon.Error);
                    throw;
                }                
            }
            finally
        ...
        private void registContractorRtnPackingLineTBL(int packingCnt)
        {
            ...
            try
            {
                ...
                int maxId = dao.Select_MaxID_T_MS_PB_PACKING_LINE();
                ...
                dao.Insert_ContractorRtn_T_MS_PB_PACKING_LINE(plEntity);
            ...
            catch (Exception ex)
            {
                // ロールバック
                dao.Rollback();
                _logger.Info("梱包ラインテーブル登録処理でエラーが発生しました。");
                _logger.Info("エラー内容：" + ex.Message);
                new clsMessageBox().MessageBoxOKOnly("梱包ラインテーブル登録処理でエラーが発生しました。" + Environment.NewLine + ex.Message.ToString() + Environment.NewLine + "開発部門に問い合わせ願います。", "エラー", MessageBoxIcon.Error);
                throw;
            }
            finally
            {
        ...
        /// </summary>
        /// <returns></returns>
        private void updateEscalationTBL()
        {
            ...
            catch (Exception ex)
            {
                // ロールバック
                dao.Rollback();
                _logger.Info("エスカレテーブル更新処理でエラーが発生しました。");
                _logger.Info("エラー内容：" + ex.Message);
                new clsMessageBox().MessageBoxOKOnly("エスカレテーブル更新処理でエラーが発生しました。" + Environment.NewLine + ex.Message.ToString() + Environment.NewLine + "開発部門に問い合わせ願います。", "エラー", MessageBoxIcon.Error);
                throw;
            }
            finally
            {
        ...
        private void setProcessHistoryTbl(string processText, string outFileName, int outCount)
        {
            ...
            catch (Exception ex)
            {
                // ロールバック
                dao.Rollback();                
                _logger.Info("処理履歴登録処理でエラーが発生しました。");
                _logger.Info("エラー内容：" + ex.Message);
