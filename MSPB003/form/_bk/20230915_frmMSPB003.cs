    ...
    class frmMSPB003 : Form
    {
        ...
        private void outputShippingSlip()
        {
            ...
            try
            {
                ...
                var yuPackCnt = string.IsNullOrEmpty(yuPackChekFile) ? 0 : 1;

                // 終了メッセージ
                new clsMessageBox().MessageBoxOKOnly("発送帳票出力が完了しました。" + Environment.NewLine +
                                                     $"ＭＳ返却出力対象件数　： {msRtnCnt}件" + Environment.NewLine +
                                                     $"契約者返却出力対象件数： {conRtnCnt + checkListCnt + yuPackCnt}件" , "完了", MessageBoxIcon.Information);
                this.Cursor = Cursors.WaitCursor;
